...
namespace Mine.NET.metadata
{
    ...
    public class LazyMetadataValue : MetadataValueAdapter, MetadataValue
    {
        ...
        private WeakReference<Object> internalValue;
         ...
         */
        public LazyMetadataValue(Plugin owningPlugin, Func<Object> lazyValue) : this(owningPlugin, CacheStrategy.CACHE_AFTER_FIRST_EVAL, lazyValue)
        {
        }
         ...
         * @param lazyValue the lazy value assigned to this metadata value.
         */
        public LazyMetadataValue(Plugin owningPlugin, CacheStrategy cacheStrategy, Func<Object> lazyValue) : base(owningPlugin)
        {
            if (lazyValue == null) throw new ArgumentNullException("lazyValue cannot be null");
            this.internalValue = new WeakReference<Object>(null);
         ...
         */
        protected LazyMetadataValue(Plugin owningPlugin) : base(owningPlugin)
        {
        }
        ...
        public override Object value()
        {
            ...
            if (!internalValue.TryGetTarget(out value) || value == ACTUALLY_NULL)
            {
                return null;
            }
            return value;
        ...
        private void eval()
        {
            ...
            if (cacheStrategy == CacheStrategy.NEVER_CACHE || !internalValue.TryGetTarget(out target)) //TODO
            {
                try
                {
                    ...
                    internalValue = new WeakReference<Object>(value);
                }
                catch (Exception e)
                {
        ...
        public override void invalidate()
        {
            ...
            {
                internalValue.SetTarget(ACTUALLY_NULL);
            }
        }
