...
namespace Mine.NET.util
{
    ...
    public class BlockIterator : IEnumerator<Block>
    {
        ...
        public BlockIterator(World world, Vector start, Vector direction, double yOffset, int maxDistance)
        {
            ...
            lastBlock = startBlock.getRelative(BlockFace.getOppositeFace(mainFace));

            if (secondError < 0)
            {
                secondError += gridSize;
                lastBlock = lastBlock.getRelative(BlockFace.getOppositeFace(secondFace));
            }

            if (thirdError < 0)
            {
                thirdError += gridSize;
                lastBlock = lastBlock.getRelative(BlockFace.getOppositeFace(thirdFace));
            }
